...
?>
	...
	<div class="hk-row-fluid">
		...
		<div class="wizard_subdiv_right hkc-md-9">
			...
			<?php
			$values = array(
				JHTML::_('select.option', 1, JText::_('HIKA_SANDBOX')),
				JHTML::_('select.option', 0,JText::_('HIKA_PRODUCTION'))
			);
			echo JHTML::_('hikaselect.radiolist',  $values, 'sanbox', 'class="custom-select"', 'value', 'text', 0 );
	?>
				<div id="wizard_paypal_connect_button">
				<?php
				$plugin = hikashop_import('hikashoppayment', 'paypalcheckout');
				echo $plugin->getConnectButtonHTML(false, 'window.hikashop.toggleButton');
				?>
				</div>
				<script>
					window.hikashop.toggleButton = function() {
						document.getElementById('wizard_paypal_connect_button').innerHTML = '<?php echo hikashop_display(JText::_('PAYPAL_CHECKOUT_SUCCESSFULLY_CONNECTED'),'success',true); ?>';
					}
				</script>
				<input type="hidden" name="paypal_client_id" id="data_payment_payment_params_client_id" value=""/>
				<input type="hidden" name="paypal_client_secret" id="data_payment_payment_params_client_secret" value=""/>
				<input type="hidden" name="paypal_merchant_id" id="data_payment_payment_params_merchant_id" value=""/>
			</div>
		</div>
	</div>
	<div style="clear:both;"></div>
