...
class plgButtonHikashopproduct extends JPlugin
{
	...
	function onDisplay($name, $asset='', $author='') {
		...
		if(!in_array($extension, array('com_content', 'com_advancedmodules', 'com_tz_portfolio', 'com_k2', 'com_jevents', 'com_modules')))
			return;
		if(!defined('DS'))
		...
		if (	$user->authorise('core.edit', $asset)
			...
			if(HIKASHOP_J40) {
				...
				$button->set('icon', 'cube');
				$button->set('iconSVG', '<svg viewBox="0 0 512 512" width="24" height="24"><path d="M239.1 6.3l-208 78c-18.7 7-31.1 '
...
?>
	...
	<form action="<?php echo hikashop_currentURL();?>" method="POST" name="adminForm" id="adminForm">
		<table class="hikashop_no_border">
			...
			<button class="btn btn-success" onclick="checkSelect(); insertTag(document.getElementById('product_insert').value, event);" style="vertical-align:top;float:right;">Insert</button>
		</table>
		...
		<button class="btn btn-success" onclick="checkSelect(); insertTag(document.getElementById('product_insert').value, event);" style="margin-top:5px;float:right;">Insert</button>
		<?php global $Itemid; ?>
